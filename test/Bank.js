...
describe("Bank contract", function () {
  ...
  it("deposit ERC20 to account", async function () {
    ...
    expect(await patlom.tokenToAmount(token.address)).to.equal(
  ...
  it("transfer to another account by account's name", async function () {
    ...
    // expect it to have token with same amount - zero amount
    expect(await patlom.tokenToAmount(token.address)).to.equal(initialBalance);
    expect(await bounk.tokenToAmount(token.address)).to.equal(
      constants.ZERO_ADDRESS
    );
    // do transfer
    await bank
    ...
    // expect it to has zero amount - same amount
    expect(await patlom.tokenToAmount(token.address)).to.equal(0);
  ...
  it("fee(1%) when transfer between 2 different account ", async function () {
    // create mock account from owner(accounts[0])
    await bank.connect(owner).createAccount("Patlom");
    address1 = await bank.accounts("Patlom");
    patlom = await ethers.getContractAt("Account", address1);
    // create mock account from accounts[1] ## different acccount
    ...
    // calculate for amount - 1%
    let decAmount = ethers.utils.formatEther(initialBalance);
    // remainingAmount = initialBalance - 1%
    let remainingAmount = decAmount - decAmount / 100;
    // format back to Bignumber
    remainingAmount = ethers.utils.parseUnits(remainingAmount.toString(), 18);
    // does it equal to amount in bounk account
    expect(await bounk.tokenToAmount(token.address)).to.equal(remainingAmount);
  });
  ...
  it("multiple transfer ", async function () {
    ...
    expect(await beam.tokenToAmount(token.address)).to.equal(
      constants.ZERO_ADDRESS
    );
    expect(await bounk.tokenToAmount(token.address)).to.equal(
      constants.ZERO_ADDRESS
    ...
    await bank
      .connect(owner)
      .multipleTransfer("Patlom", tokenList, toList, amountList);
    // expect it to has zero amount
    expect(await patlom.tokenToAmount(token.address)).to.equal(0);
    // this one is 1% off
    expect(await bounk.tokenToAmount(token.address)).to.not.equal(
      initialBalance
    );
